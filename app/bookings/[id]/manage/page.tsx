  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {cancelSuccess && (
          <div className="mb-6 bg-green-50 border border-green-100 rounded-lg p-4 shadow-sm">
            <div className="flex">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2">
            <div className="bg-white shadow-md rounded-lg overflow-hidden mb-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-4 shadow-sm">
                  ...
                  <div className="flex items-center">
                    <div className="text-right mr-4">
                      <p className="font-bold text-lg text-gray-900">{booking.departureTime}</p>
                      <p className="text-gray-600 text-sm">{booking.origin}</p>
                      {flightStatus?.departureGate && (
                        <p className="text-xs text-indigo-600">Gate {flightStatus.departureGate}</p>
                      )}
                    </div>
                    ...
                    <div className="flex-grow mx-2 px-2">
                      <div className="relative flex items-center justify-center">
                        ...
                        {flightStatus?.status === 'delayed' && (
                          <div className="absolute text-amber-600 text-xs whitespace-nowrap">
                    ...
                    <div className="text-left ml-4">
                      <p className="font-bold text-lg text-gray-900">{booking.arrivalTime}</p>
                      <p className="text-gray-600 text-sm">{booking.destination}</p>
                      {flightStatus?.arrivalGate && (
                        <p className="text-xs text-indigo-600">Gate {flightStatus.arrivalGate}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {booking.returnFlightNumber && booking.returnDepartureDate && (
                  <div className="bg-gray-50 rounded-lg p-4 mt-4 shadow-sm">
                    <div className="flex items-center mb-2">
                      <span className="bg-indigo-50 text-indigo-700 text-xs font-semibold px-2.5 py-0.5 rounded-full">Return Flight</span>
                    </div>
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4">
                      <div>
                        <p className="text-sm text-gray-500">Flight</p>
                        <p className="font-medium text-gray-900">{booking.returnFlightNumber}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Date</p>
                        <p className="font-medium text-gray-900">{formatDate(booking.returnDepartureDate)}</p>
                      </div>
                    </div>
                    
                    <div className="flex items-center">
                      <div className="text-right mr-4">
                        <p className="font-bold text-lg text-gray-900">{booking.returnDepartureTime}</p>
                        <p className="text-gray-600 text-sm">{booking.destination}</p>
                      </div>
                      ...
                      <div className="text-left ml-4">
                        <p className="font-bold text-lg text-gray-900">{booking.returnArrivalTime}</p>
                        <p className="text-gray-600 text-sm">{booking.origin}</p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-6">
                <h3 className="text-md font-semibold text-gray-800 mb-3">Passengers</h3>
                <div className="space-y-2">
                  {booking.passengers.map((passenger, index) => (
                    <div key={passenger.id} className="bg-gray-50 p-3 rounded-lg shadow-sm">
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3 max-h-80 overflow-y-auto">
                    {statusUpdates.map((update, index) => (
                      <div key={index} className="p-3 bg-gray-50 rounded-lg shadow-sm border-l-4 border-indigo-500">
                ...
                <div className="space-y-4">
                  ...
                  {isBookingModifiable && booking.checkInStatus === 'available' && (
                    <Link
                      ...
                      className="w-full flex justify-center py-2 px-4 border border-indigo-600 rounded-md shadow-sm text-sm font-medium text-indigo-600 bg-white hover:bg-indigo-50 focus:outline-none transition-colors"
                    >
                      Online Check-in
                    </Link>
                  )}
                  ...
                  {canCancel && (
                    <button
                      ...
                      className="w-full flex justify-center py-2 px-4 border border-red-500 rounded-md shadow-sm text-sm font-medium text-red-500 bg-white hover:bg-red-50 focus:outline-none transition-colors"
                    >
                      Cancel Booking
                    </button>
                  )}
              ...
              <div className="p-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="mt-1 text-gray-900">1-800-SKY-HELP (1-800-759-4357)</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Email</h3>
                    <p className="mt-1 text-gray-900">support@skyvoyager.com</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Hours</h3>
                    <p className="mt-1 text-gray-900">24/7 for urgent travel matters</p>
                  </div>
                </div>
              </div>
            </div>
